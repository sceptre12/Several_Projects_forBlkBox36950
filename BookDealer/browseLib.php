<!DOCTYPE html>
<html>
	<head>
		<meta charset="utf-8" />
		<title>App</title>
		<link rel="stylesheet" href="http://code.jquery.com/mobile/1.4.3/jquery.mobile-1.4.3.min.css" />
		<link rel="stylesheet" type="text/css" href="css/Blue.min.css">
		<script src="http://code.jquery.com/jquery-1.11.1.min.js"></script>
		<script src="http://code.jquery.com/mobile/1.4.3/jquery.mobile-1.4.3.min.js"></script>
		<link rel="stylesheet" type="text/css" href="css/app.css">
		<meta name="viewport" content="width=device-width, intial-scale=1, maximum-scale=1"/>
	</head>
	<body>
		<div data-role="page" id="browseLib">
			<header data-role="header">
				<h1>Enter Search Item</h1>
				<a href="mainScreen.php" data-icon="bars" data-iconpos="notext" class="ui-nodisc-icon">Back</a>
				
			</header>
			<div data-role="main">
				<div id="blib">
					<ul data-role="listview" class="ui-listview" data-filter="true" data-filter-reveal="true" data-filter-placeholder="Browse Library...">
					<li>
						<a href="userProfile.html">
						<img src="img/img1.jpg" class="ui-li-thumb">
						<h1>Name of Book</h1>
						<p>ISBN Number:</p>
						<p>Adrress</p>
						</a>
					</li>
					<li>
						<a href="userProfile.html">
						<img src="img/img1.jpg" class="ui-li-thumb">
						<h1>Name of Book</h1>
						<p>ISBN Number:</p>
						<p>Adrress</p>
						</a>
					</li>
					<li>
						<a href="userProfile.html">
						<img src="img/img1.jpg" class="ui-li-thumb">
						<h1>Name of Book</h1>
						<p>ISBN Number:</p>
						<p>Adrress</p>
						</a>
					</li>
					<li>
						<a href="userProfile.html">
						<img src="img/img1.jpg" class="ui-li-thumb">
						<h1>Name of Book</h1>
						<p>ISBN Number:</p>
						<p>Adrress</p>
						</a>
					</li>
					<li>
						<a href="userProfile.html">
						<img src="img/img1.jpg" class="ui-li-thumb">
						<h1>Name of Book</h1>
						<p>ISBN Number:</p>
						<p>Adrress</p>
						</a>
					</li>
					<li>
						<a href="userProfile.html">
						<img src="img/img1.jpg" class="ui-li-thumb">
						<h1>Name of Book</h1>
						<p>ISBN Number:</p>
						<p>Adrress</p>
						</a>
					</li>
					<li>
						<a href="userProfile.html">
						<img src="img/img1.jpg" class="ui-li-thumb">
						<h1>Name of Book</h1>
						<p>ISBN Number:</p>
						<p>Adrress</p>
						</a>
					</li>
					<li>
						<a href="userProfile.html">
						<img src="img/img1.jpg" class="ui-li-thumb">
						<h1>Name of Book</h1>
						<p>ISBN Number:</p>
						<p>Adrress</p>
						</a>
					</li>
					<li>
						<a href="userProfile.html">
						<img src="img/img1.jpg" class="ui-li-thumb">
						<h1>Name of Book</h1>
						<p>ISBN Number:</p>
						<p>Adrress</p>
						</a>
					</li>
					<li>
						<a href="userProfile.html">
						<img src="img/img1.jpg" class="ui-li-thumb">
						<h1>Name of Book</h1>
						<p>ISBN Number:</p>
						<p>Adrress</p>
						</a>
					</li>
					<li>
						<a href="userProfile.html">
						<img src="img/img1.jpg" class="ui-li-thumb">
						<h1>Name of Book</h1>
						<p>ISBN Number:</p>
						<p>Adrress</p>
						</a>
					</li>
					<li>
						<a href="userProfile.html">
						<img src="img/img1.jpg" class="ui-li-thumb">
						<h1>Name of Book</h1>
						<p>ISBN Number:</p>
						<p>Adrress</p>
						</a>
					</li>
					<li>
						<a href="userProfile.html">
						<img src="img/img1.jpg" class="ui-li-thumb">
						<h1>Name of Book</h1>
						<p>ISBN Number:</p>
						<p>Adrress</p>
						</a>
					</li>
				</div>
				</ul>			
			</div>
		</div>
	</body>
</html>